...
for nr=1:NR,
   ...
   BASE = RECTS(:,(nr-1)*4+[1:4]);
   rect = [BASE(:,1:2),zeros(126,1),zeros(126,1)] + round(FR .* [BASE(:,3:4),BASE(:,3:4)]);

   % Default coordinates: one rectangle
   xcoords = max(1,min(size(II,2),rect(:,1)+[zeros(126,1),rect(:,3)]));
   ycoords = max(1,min(size(II,1),rect(:,2)+[zeros(126,1),rect(:,4)]));
   xcofs = [-1,1];
   ycofs = [-1,1];

   % Coordinates from which to sample II
   if (((ORDER==2) && (VERT==1)) || (ORDER==4)),
     xcoords = max(1,min(size(II,2),rect(:,1)+round([zeros(126,1),ones(126,1)*0.5,ones(126,1)*1]*rect(:,3))));
     xcofs = [1,-2,1];
   end
   if (((ORDER==2) && (VERT==0)) || (ORDER==4)),
     ycoords = max(1,min(size(II,1),rect(:,2)+round([ones(126,1)*0,ones(126,1)*0.5,ones(126,1)*1]*rect(:,4))));
     ycofs = [1,-2,1];
   end
   if ((ORDER==3) && (VERT==1)),
     xcoords = max(1,min(size(II,2),rect(:,1)+round([ones(126,1)*0,ones(126,1)*0.33,ones(126,1)*0.67,ones(1,126)*1]*rect(:,3))));
     xcofs = [-1,2,-2,1];
   end
   if ((ORDER==3) && (VERT==0)),
     ycoords = max(1,min(size(II,1),rect(:,2)+round([ones(126,1)*0,ones(126,1)*0.3,ones(126,1)*0.67,ones(1,126)*1]*rect(:,4))));
     ycofs = [-1,2,-2,1];
   end

   % Sample the integral image
   for k = 1:K
     F(k,nr) = F(k,nr) + ycofs*II(ycoords(k,:),xcoords(k,:),k)*xcofs';
   end
end
